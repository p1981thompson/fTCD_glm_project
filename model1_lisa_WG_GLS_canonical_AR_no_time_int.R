...
# Create convolved stimulus function with HRF (applying the new fmri.stimulus.PT2 function above)
    ...
    canonical1 = fmri.stimulus.PT2(scans = dim(rawdata)[1], onsets = c(1,1+which(diff(rawdata$stim1_on)!=0)), durations = stim1_length_samples, TR = 1,scale=1)
    
    canonical2 = fmri.stimulus.PT2(scans = dim(rawdata)[1], onsets = c(1,1+which(diff(rawdata$stim2_on)!=0)), durations = stim2_length_samples, TR = 1,scale=1)
    ...
    canonical = as.matrix(cbind(canonical1,canonical2))
    
    canonical = rbind(canonical,canonical)
    ...
    my_des<-fmri.design(canonical, order = order)
    
    # Add a dummy variable for side (signal). This is either 0 or 1 for left and right respectively.
    my_des<-cbind(my_des,rep(1:0,each=length(canonical1)))
    ...
    #-----------------------------------------------------------------------------------------------#
 #some data wrangling operations to format data for model.   
   ...
   # Filter data to remove dependent variable duplicates due to averaging from heartbeat correction. 
    mydata<-mydata %>% group_by(y) %>% filter(t==min(t))
    
    # Set nmle::gls() optimisation parameters.
    glsControl(optimMethod = "L-BFGS-B",maxIter = 100)
    
    # fits model using generalized least squares which allows for correlated residuals.
    ...
    glm.data[j,(((order+5)*1)+2)] <- "canonical"
  ...
  # create plot data  
    ...
    #plots time series for each individual with their respective models  
    ...
    print(g3)
    #Also, plotting the autocorrelation plot of the residuals from the model.
...
pdf(file = '/Users/paulthompson/Documents/fTCD_glm_results/Lisa_data/test models/HRF_signals_plots_LISA_WG_GLS_canonical_AR1.pdf', onefile = TRUE) #print plots to file.
my_results_LISA_WG_GLS_canonical_AR1<-fTCD_glm_LISA_GLS(path='/Users/paulthompson/Documents/fTCD_glm_results/Lisa_data/Chpt4_fTCD_WordGen_rawdata',order=order)
dev.off()
#--------------------------------------------------------------------------------------------------#
write.csv(my_results_LISA_WG_GLS_canonical_AR1,'/Users/paulthompson/Documents/fTCD_glm_results/Lisa_data/test models/my_results_LISA_WG_GLS_canonical_AR1_time_int.csv',row.names = FALSE)
my_results_LISA_WG_GLS_canonical_AR1<-my_results_LISA_WG_GLS_canonical_AR1[complete.cases(my_results_LISA_WG_GLS_canonical_AR1), ]
#--------------------------------------------------------------------------------------------------#
...
# 
my_results_LISA_WG_GLS_canonical_AR1_ex <- my_results_LISA_WG_GLS_canonical_AR1[!my_results_LISA_WG_GLS_canonical_AR1$ID %in% exclude_id,]
# 
# 
#------------------------------------------------------------------------------------------------#
# Plot script for autocorrelation plot (fmri, ftcd old and ftcd new)
#
#------------------------------------------------------------------------------------------------#
...
compare_results<-merge(my_results_LISA_WG_GLS_canonical_AR1_ex,old_res,by='ID',all.x = T)
...
pdf(file = '/Users/paulthompson/Documents/fTCD_glm_results/Lisa_data/test models/HRF_signals_plots_LISA_WG_GLS_canonical_AR1_correlations.pdf')
