...
pdf(file = 'HRF_signals_plots_multi_stim_holly_LI1.pdf', onefile = TRUE) # print plots to file
my_results_multi2A<-fTCD_glm_multi(path=paste0(getwd(),"/Holly_fTCD_data_run1"),order=order)
dev.off()
...
mylong_resultsM2A<-gather(my_results_multi2A,key='param',value='beta',-c(ID,HRF,p_value_contrast))

names(mylong_resultsM2A)[2]<-"HRF"


ggplot(mylong_resultsM2A,aes(x=beta))+geom_density(fill='blue',alpha=0.5)+facet_wrap(~param,scales='free')+theme_bw()
...
compare_resultsM2A<-merge(my_results_multi2A,old_res_holly,by='ID',all.x = T)


psych::pairs.panels(compare_resultsM2A[,names(compare_resultsM2) != c('ID','HRF','p_value_contrast')],cex.cor=3)

psych::pairs.panels(compare_resultsM2A[, c('param12','param13','param14',"Run1_SG","Run1_WG","Run1_LG")],cex.cor=1)
