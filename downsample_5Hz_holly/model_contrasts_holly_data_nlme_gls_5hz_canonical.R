...
# Downsampled to 5Hz.
# Fixed double Gamma (canonical) HRF
# nlme::gls( AR(1)) model 
...
fTCD_glm_multi<-function(path,order)
{
  ...
  for(j in 1:length(filename1))
  {
    ...
    # Adapted 'fmri.stimulus' function from the R package 'fmri'. This is a condensed version that only gives option of the double gamma (canonical) HRF and convolves the HRF to the stimli specificied earlier in this script
