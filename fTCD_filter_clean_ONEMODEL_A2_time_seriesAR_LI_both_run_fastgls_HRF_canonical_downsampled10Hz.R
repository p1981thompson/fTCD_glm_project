...
fTCD_glm_A2_rcpp<-function(path,order)
{
  ...
  for(j in 1:length(filename1))
  {
    ...
    gamma1 = fmri.stimulus.PT3(scans = dim(rawdata)[1], onsets = c(1,1+which(diff(rawdata$stim1_on)!=0)), durations = stim1_length_samples, TR = 1,scale=1,epoch_data = myepoched_average,startend<-c(0,17))
...
pdf(file = 'HRF_signals_plots_A2project_SG_rcpp_downsamp10hz.pdf', onefile = TRUE) #print plots to file.
#start_time = Sys.time()
my_results_A2_SG_Downsamp5Hz<-fTCD_glm_A2_rcpp(path="/Volumes/PSYHOME/PSYRES/pthompson/DVMB/fTCD_glm_project/A2_SG_data",order=order)
#end_time = Sys.time()
