...
fTCD_glm4<-function(path,order)
{
  ...
  for(j in 1:length(filename1))
  {
    ...
    gamma1 = fmri.stimulus.PT2(scans = dim(rawdata)[1], onsets = c(1,1+which(diff(rawdata$stim1_on)!=0))[seq(2, length(c(1,1+which(diff(rawdata$stim1_on)!=0))), by = 2)], durations = stim1_length_samples, TR = 1,scale=1)
...
pdf(file = 'HRF_signals_plots_PPTT_comb_stim.pdf', onefile = TRUE) #print plots to file
